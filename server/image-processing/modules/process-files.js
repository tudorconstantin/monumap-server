...
// ////////////////////////////////////////////////////////////////////////////////////////////
// // EXPORTS
module.exports = (inPath, outPath, userName, monArr) => {
    // images counter
    let imageCounter = 0;
    ...
    monArr.forEach((item, index) => {
        // create path string
        const itemPath = `${item[1]}_${item[3]}/${item[4]}_${item[5]}/${item[6]}_${item[7].replace(' ', '-')}/${item[0]}`;
        ...
        // read files
        const fileName = `*_B-II-[am]-[AB]-${item[15]}?(_?).@(JPG|jpg)`;
        // test if file exists
        if (glob.sync(fileName, { cwd: inPath }).length > 0) {
            ...
            for (let i = 0; i < matchedFileNames.length; i += 1) {
                ...
                const newJPEGFileName = `${item[0]}_${dateString}_${userName}_${indexString}`;
                // console.log(newJPEGFileName);
                const newJPEGFilePath = `${itemFullPath}/${newJPEGFileName}.jpg`;
