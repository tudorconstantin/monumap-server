...
export default {
  ...
  async created() {
    ...
    const lie2020 = await fetch('/geojson/lie-2020-1.geojson').then((r) =>
      r.json()
    );
    ...
    features = features.concat(
      (lie2020?.features || [])
        ...
        .map((i) => {
          ...
          i.properties.source = 'LIE_2020_1';
          i.properties.denumire = `${i.properties.expert_teh} - ${i.properties.adr_strada} nr ${i.properties.adr_nr} (${i.properties.adr_nr_obs || ''} an expertiza: ${i.properties.an_experti})`
          return i;
        })
    );
    ...
    async selectItem(polygonProps) {
      ...
      const fullPolygon = this.geoJSON.features.find(
        (m) =>
          ...
          m['properties']['api_name'] === polygonProps['api_name']
      );
